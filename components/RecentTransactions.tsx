...
const RecentTransactions = ({
  ...
  const [categorizedTransactions, setCategorizedTransactions] = React.useState(transactions);

  React.useEffect(() => {
    const categorize = async () => {
      // Dynamically import to avoid SSR issues
      const { categorizeTransactions } = await import("@/lib/actions/user.actions");
      const result = await categorizeTransactions(transactions);
      setCategorizedTransactions(result);
    };
    categorize();
  }, [transactions]);
